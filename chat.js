...
module.exports = async (req, res) => {
  ...
  try {
    ...
    console.log('Sending request to Chatbase:', {
      ...chatbaseRequest,
      chatbotId: CHATBOT_ID.substring(0, 8) + '...' // Log partial ID for security
    });
    ...
    if (stream) {
      ...
      try {
            ...
            if (data === '[DONE]') {
              ...
              try {
                ...
                if (content) {
                  ...
                  // Send each chunk in Proto Persona's expected format
        ...
        res.write(`data: ${JSON.stringify({ error: 'Failed to connect to Chatbase' })}\n\n`);
        res.end();
      }
    } else {
      ...
      try {
        ...
        } else if (chatbaseResponse.choices && chatbaseResponse.choices[0]) {
          responseText = chatbaseResponse.choices[0].message?.content || '';
        } else if (chatbaseResponse.answer) {
          responseText = chatbaseResponse.answer;
        }

        if (!responseText) {
          console.error('Unexpected Chatbase response structure:', chatbaseResponse);
          return res.status(500).json({ 
            error: 'Invalid response from Chatbase' 
          });
        }

        // Return response in Proto Persona's expected format
        res.status(200).json({ response: responseText });

      } catch (apiError) {
        console.error('Chatbase API error:', apiError.response?.data || apiError.message);
        
        // Handle specific error cases
        if (apiError.response?.status === 401) {
          return res.status(401).json({ 
            error: 'Authentication failed. Please check your Chatbase API key.' 
          });
        } else if (apiError.response?.status === 404) {
          return res.status(404).json({ 
            error: 'Chatbot not found. Please check your chatbot ID.' 
          });
        } else {
          return res.status(500).json({ 
